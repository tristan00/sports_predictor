...
class DataManager():
    ...
    def __init__(self, encoder_size = 64, history_size = 16):
    ...
    def build_past_n_game_dataset(self):
        ...
        for t in tqdm.tqdm(teams):
            ...
            for g in game_ids:
                pregame_matrices = []
                for n in range(1, self.history_size + 1):
                    pregame_matrices.append(temp_team_data.shift(n).loc[g, self.initial_team_data_columns])
    ...
    def calculate_team_game_rating(self, rating_type):
        ...
        team_data_copy[new_col] = None
        team_data_copy[f'{self.feature_indicator_str }_{self.team_str}_{self.postgame_rating_str}_{rating_type}'] = None

        for i, r in tqdm.tqdm(self.team_data.iterrows()):
            team_previous_record = self.get_most_recent_team_record_before_date(team_data_copy, r['team_tag'], r['date_str'])
            opponent_previous_record = self.get_most_recent_team_record_before_date(team_data_copy, r['opponent_tag'], r['date_str'])
            ...
            team_data_copy.loc[i, new_col] = team_previous_rating
            team_data_copy.loc[i, f'{self.feature_indicator_str }_{self.team_str}_{self.postgame_rating_str}_{rating_type}'] = get_new_rating(team_previous_rating,
                                                                                                                                   opponent_previous_rating,
                                                                                                                                   r['win'], multiplier = 1, rating_type = rating_type)

        self.game_rating_data = team_data_copy[['team_tag', 'opponent_tag', 'date_str', 'game_key', 'team_game_key', new_col]]
